...
def ensemble_maps(input_folder_list, output_dir):
    ...
    for conf_map_name in os.listdir(input_folder_list[0]):
        ...
        for input_folder in input_folder_list:
            ...
            cur_conf_map = io.imread(cur_conf_map_name)
            # Sum the conf maps
            if sum_conf_map is None:
                sum_conf_map = cur_conf_map
            else:
                sum_conf_map += cur_conf_map
    ...
    try: 
        ...
        for i in range(1, 5):
            ...
            for j in range(1, 5):
                ...
                output_dir = os.path.join(mother_dir, 'd{}/images/'.format(i), 'ensembled_img_d{}_model_d{}'.format(i, j))
                folder_list = []
